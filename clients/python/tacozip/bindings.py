...
def _normalize_inputs(
    src_files: List[Union[str, pathlib.Path]], arc_files: List[str] = None
    ...
    if arc_files is not None:
        if len(arc_files) != len(normalized_src):
            raise ValueError(
                f"Archive names count ({len(arc_files)}) must match source files count ({len(normalized_src)})"
            )
...
def create(
    ...
    result = _lib.tacozip_create(
        validated_zip_path.encode("utf-8"),
        src_array,
        arc_array,
        len(normalized_src),
        ctypes.byref(meta),
